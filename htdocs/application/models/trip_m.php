...
*/
    ...
    public function get_uninvited_users_by_user_id($user_id = NULL)
    {
        ...
        if ($uninvited_user_ids === FALSE)
        {
            ...
            $sql = 'SELECT ruu.related_user_id FROM `users_users` ruu WHERE ruu.user_id = ? AND ruu.related_user_id NOT IN (SELECT user_id FROM `trips_users` WHERE trip_id = ? AND role >= 5)';
